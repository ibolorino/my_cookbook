...
class TestCreateIngredient(TestIngredientBase):
    ...
    def test_success_owner_id(self, recipe_item_mock):
        ...
        ingredient_in = schemas.IngredientCreate(
            name="batata", quantity="500g", recipe_item_id=1
        )
...
class TestUpdateIngredient(TestIngredientBase):
    ...
    def test_success_owner_id(self, ingredient_mock):
        ...
        ingredient_mock.return_value = models.Ingredient(
            id=1, name="batata", quantity="500g", recipe_item=recipe_item
        )
        response = update_ingredient(
            db=self.db, id=1, ingredient_in=ingredient_in, current_user=current_user
        )
        self.assertTrue(response.name == ingredient_in.name)
...
class TestDeleteIngredient(TestIngredientBase):
    ...
    def test_success_owner_id(self, ingredient_mock):
        ...
        response = delete_ingredient(
            db=self.db, id=ingredient_id, current_user=current_user
        )
